...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var objSamples = samplesArr.filter(sampleObj => sampleObj.id == sample);
    var objMetadata = metadataArr.filter(metaObj => metaObj.id == sample);
    //  5. Create a variable that holds the first sample in the array.
    var sampleResult = objSamples[0];
    var metadataResult = objMetadata[0];
    ...
    var xvalues = sampleValues.slice(0.10).map(val => val);
    ...
    var gaugeData = [{
      ...
      gauge: {
        axis:{range:[0,10], tickcolor:'dimgray'},
        bar:{color:'black'},
        bordercolor: 'dimgray',
        steps: [
